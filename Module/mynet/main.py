...
class Backward(nn.Module):
    def __init__(self, n_gauss=25):
        ...
        self.pai_net = nn.Linear(200, 3)
        self.param = nn.ModuleList([nn.Linear(200, 4) for i in range(2 * n_gauss)])
    ...
    def forward(self, x0, pretrain=None, n_samples=10):
        ...
        mu_list = []
        sigma_list = []
        for i in range(0, len(self.param), 2):
            mu_list.append(self.param[i](x))
            sigma_list.append(torch.abs(self.param[i + 1](x)))
        
        mu = torch.stack(mu_list, dim=1)  # 形状为 (batch_size, num_gaussians, output_dim)
